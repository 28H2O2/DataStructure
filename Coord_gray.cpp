...
void Coord_gray::printresults()
{
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            if(dot_canvas[i][j] == 19)
              cout << "w";
            else if(dot_canvas[i][j] == 24)
              cout << "b";
            else if(dot_canvas[i][j] != 0)
...
void Coord_gray::dot(const int x, const int y, const int color)
{
  if(color != 0)
    dot_canvas[height - 1 - y][x] = color;
  else
    dot_canvas[height - 1 - y][x] = 10;    
}

void Coord_gray::clear()
{
  for (int i = 0; i < height; i++)
  {
      for (int j = 0; j < width; j++)
